...
const getClosestConfirmedDistance = (
  a: ClosedConfirmedDistance,
  b: ClosedConfirmedDistance
...
const writeIntruderPilots = async () => {
  ...
  await Promise.all(
    intrudersData.map(async pilot => {
      ...
      const closedConfirmedDistanceToWrite: ClosedConfirmedDistance =
        dbData && dbData.closedConfirmedDistance
          ? getClosestConfirmedDistance(dbData.closedConfirmedDistance, closedConfirmedDistance)
          : closedConfirmedDistance;

      console.info(`Write ${firstName} ${lastName} to intruders database`);
      console.log(
        `with closed confirmed distance: ${Number(closedConfirmedDistanceToWrite.distance / 1000).toFixed(2)} meters`
      );
