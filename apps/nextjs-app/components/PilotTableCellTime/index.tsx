...
const PilotTableCellTime: React.FunctionComponent<PilotTableCellTimeProps> = ({ time }) => {
  ...
  return (
    <div className={color(minutes)}>{Math.floor(seconds) < 60 ? `less than minute ago` : `${minutes} minutes ago`}</div>
  );
};
